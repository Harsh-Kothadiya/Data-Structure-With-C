...
struct QUEUE* ENQUEUE(struct QUEUE *tmp,int val)
{
    ...
    if( (tmp->front==0&&tmp->rear==tmp->size-1) || (tmp->front==tmp->rear+1) )
    {
        ...
        {
            for(i=tmp->front;i<oldSize;i++)
            {
                tmpArray[i]=tmp->ptr[i];
            }
        }
        else  // or if( tmp->front > tmp->rear )
        {
            j=0;
            for(i=tmp->front;i<oldSize;i++)
            {
                tmpArray[j]=tmp->ptr[i];
                j++;
            }
        ...
        free(tmp->ptr);
        tmp->ptr=(int*)malloc(sizeof(int)*s1);
        for(i=tmp->front;i<=tmp->rear;i++)
        {
            tmp->ptr[i]=tmpArray[i];
        }
        tmp->rear++;
    ...
    else if( ((tmp->rear==tmp->size-1)&&(tmp->front!=0)) )
    {
        tmp->rear=0;
        tmp->ptr[tmp->rear]=val;
        printf("\n\n ENQUEUE VALUE : %d && INDEX : %d\n\n",val,tmp->rear);
